...
export default function LinePlot({ title, yTitle, plotData }: PlotProps) {
  ...
  const trimPlotData = {
    ...plotData,
    data: plotData.data.reduce((trimmed: Array<{ x: Date | string; y: null | number }>, { x, y }, currInd, rawData) => {
  ...
  const timeRange = latestPoint && earliestPoint ? latestPoint?.getTime() - earliestPoint?.getTime() : 300000
