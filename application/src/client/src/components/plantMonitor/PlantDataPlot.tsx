...
export default function PlantDataPlot() {
  ...
  return (
    <Container maxWidth={false} className={classes.dataPlotContainer}>
      {plantData.isLoading ? (
        <Spinner />
      ) : plantData.data.soilMoisture.length > 0 && plantData.data.lightLevel.length > 0 && soilData ? (
        <Grid container spacing={6} style={{ padding: "25px 0" }}>
          <LinePlot title='Soil Moisture' yTitle='Soil Moisture' plotData={{ id: "Soil Moisture", data: soilData }} />
          <LinePlot title='Light Availability' yTitle='Light Level' plotData={{ id: "Light Level", data: lightData }} />
        </Grid>
      ) : (
        <NoPlantData />
      )}
